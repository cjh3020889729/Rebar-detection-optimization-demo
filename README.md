...
- 1.通过选择**更好的backbone**作为特征提取的骨干网络可以提高识别率、降低漏检率。<**MobileNetV1 到 DarkNet53**>

- 2.通过额外实验，选择理论上更好的检测架构**PPYOLO**未获得明显的完全性能提升(`Iou0.5提升不是很大`)，但也提高了检测的mmap值————即**Neck，Head部分的优化**可以提高ap。
...
- 4.一般情况通过cluster_yolo_anchor生成当前网络输入图像大小下拟合数据集的预置anchors，利用新生成的anchors替换原来的默认anchor，会使得模型预测定位上框选**位置更准确**，但是本次实验并未得到提升。原因: 由于当前数据集较小，聚类生成anchor具有局限性，anchor值太紧凑，对于训练泛化效果不好，反而容易降低ap；应在大数据集上采用anchor聚类生成相应的anchors。
